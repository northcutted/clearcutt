...
7. Review `clearcutt.yaml` before the first release. At minimum, confirm:
...
The current release workflow derives its matrix from `clearcutt.yaml` and
...
needs a different GHCR owner/repo path, update `clearcutt.yaml` and rerun
`clearcutt platform status`.
...
public trusted key come from `release.nixCache` in `clearcutt.yaml`.

## Platform Customization Points

- `clearcutt.yaml`: registry namespace, site base path, matrix, release
...
`clearcutt.yaml` if your published image names need a different prefix.
