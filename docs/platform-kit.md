...
- OCI image source/vendor labels are stamped from `core/lib/platform-metadata.nix`
  ...
  as their source.
- App rebases are signed by whichever workflow runs `clearcutt app rebase`;
  admission policy should pin that rebase workflow identity separately from the
  app developer's release workflow identity. Set it in `rebase.workflowIdentity`
  when your fork publishes one.
...
# Add and validate platform-owned service images. The reference fleet configures
# none; scaffold them if your platform publishes services alongside runtimes.
