...
- **Risk-Based Vulnerability Posture**: Production-tier images carry no *reachable, materially-risky, fixable* CVE that the platform has left unaddressed.
  - *Reachable* = present in the shipped runtime closure, enforced by `closure-cve-check` against the policy-derived in-scope set (not exploitability/call-graph reachability — a deliberately conservative, over-including proxy). For crypto (openssl/sqlite) the gate matches on a **known-good provenance/identity allowlist**, not a version floor: it substitutes the pin's patched build, VEXes the scanner version gap, and passes a shipped crypto path iff its exact store-path identity is allowlisted (an off-allowlist path fails regardless of version — default-deny preserved).
  ...
  - The thresholds live in `clearcutt.fleet.yaml` `remediation.policy` (reachability, KEV, EPSS percentile, severity floor, expiry). Forks tune the **thresholds**, never per-package waivers; `kev: always` is non-loosenable for the crypto gate. Crypto is sourced **substitute-first**: the known-good identity is the pin's patched openssl/sqlite (substituted from cache when the pin carries the fix), and the scanner version gap is cleared with VEX rather than a from-source bump. `remediation.policy.cryptoTrust` selects how that build is trusted — `nixpkgs` (default: trust the pin's patched build) or `reproduce` (also independently rebuild the crypto closure and byte-compare, failing the release evidence on divergence).
