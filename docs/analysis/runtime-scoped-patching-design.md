...
> **Update (crypto-CVE provenance reshape, Phase 1):** the gate described below
> originally matched openssl/sqlite by **version against a numeric floor**
> (`runtime-dep-floor.json` as `{dep, minVersion}`). That has since been reshaped:
> the gate now matches the shipped crypto **store-path identity** against a
> **known-good provenance allowlist** (`runtime-dep-floor.json` as
> `deps[].knownGood[].storePath` with pin + locked rev + output + version
> provenance), default-denying any off-allowlist crypto path **regardless of
> version**. A patched-not-bumped `openssl-3.6.2` with the known-good identity now
> **passes**; an unvetted higher version **fails**. Version parsing is kept only to
> recognize a crypto path and skip artifacts. The known-good build is sourced
> substitute-first (the pin's patched build) and the scanner version gap is cleared
> with VEX, rather than a from-source bump. See
> [`crypto-cve-provenance-reshape.md`](crypto-cve-provenance-reshape.md) for the
> full design. The version-floor language in the rest of this section is retained
> for historical context; read it through that reshape.
...
- **Default-deny**: any crypto store path **not on the known-good identity
  allowlist** fails (post-reshape) — this catches both stock 3.6.2 and the
  unpatched majors that exist in nixpkgs (`openssl_3_5`=3.5.6, `openssl_3`=3.0.20).
  (Originally this was framed as "any `openssl-<v>` with `v` below the floor fails";
  the default-deny posture is preserved, but membership is now identity, not
  version.)
- Recognizes a crypto path by name (accepting all 6 openssl outputs), then matches
  on **store-path identity** against the allowlist. (A hash-exact match against a
  single output would false-positive on the dev/bin/man outputs, so the allowlist
  enumerates the known-good identities.)
- Covers **slim + distroless + service** tiers (not just distroless — slim ships
  the openssl-linked runtime too), and the graft/service paths. Membership driven
  by a committed `core/tests/runtime-dep-floor.json` — now the known-good crypto
  **identity allowlist** (`deps[].knownGood[].storePath` + provenance), generated by
  `clearcutt remediation generate-crypto-allowlist` (alias `generate-floor`) from
  the `cryptoIdentities.<system>` flake output. Wired into verify.sh, `nix flake
  check`, and the pipeline evidence predicate.
