...
func TestFleetPublishCacheIncludeBuildDepsPushesBuildClosure(t *testing.T) {
	root := t.TempDir()
	cfgPath := writeFleetTestConfig(t, root, fleet.Config{
		Registry: fleet.Registry{Host: "registry.example.com", Owner: "acme", Repository: "base-images", ImagePrefix: "acme-base"},
		Matrix:   fleet.Matrix{Systems: []string{"x86_64-linux"}, Languages: []string{"coreLTS"}, Tiers: []string{"slim"}},
	...
	captureExternalOutput = func(c externalCommand) (string, error) {
		...
		switch c.Name {
		case "nix":
			if containsArg(c.Args, "--derivation") {
				return "/nix/store/ddd00000000000000000000000000000-coreLTS-slim.drv\n", nil
			}
			return "/nix/store/abc123-coreLTS-slim\n", nil
		case "nix-store":
			// requisites --include-outputs: a .drv (must be filtered out) plus
			// two realized build-closure store paths.
			return strings.Join([]string{
				"/nix/store/ddd00000000000000000000000000000-coreLTS-slim.drv",
				"/nix/store/p11kit00000000000000000000000000-p11-kit-0.26.2",
				"/nix/store/xdist000000000000000000000000000-python3.13-pytest-xdist-3.8.0",
			}, "\n") + "\n", nil
		case "aws", "curl":
			return "Sig: acme-cache-1:signed\n", nil
		default:
	...
	if _, err := runCLI(t,
		...
		t.Fatalf("publish-cache --include-build-deps error: %v", err)
	}

	var buildCopy *externalCommand
	for i := range calls {
		c := calls[i]
		if c.Name == "nix" && len(c.Args) > 0 && c.Args[0] == "copy" &&
			containsArg(c.Args, "/nix/store/p11kit00000000000000000000000000-p11-kit-0.26.2") {
			buildCopy = &calls[i]
		}
	}
	if buildCopy == nil {
		t.Fatalf("expected a nix copy carrying the build-closure paths, got:\n%s", flattenCalls(calls))
	}
	joined := strings.Join(buildCopy.Args, " ")
	if !strings.Contains(joined, "pytest-xdist-3.8.0") {
		t.Fatalf("build-closure copy missing pytest-xdist path:\n%s", joined)
	}
	if strings.Contains(joined, ".drv") {
		t.Fatalf("build-closure copy must not include .drv paths:\n%s", joined)
	}
}
