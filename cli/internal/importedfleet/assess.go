...
func WriteAssessmentDir(dir string, assessment Assessment) error {
	...
	files := map[string]any{
		"imported-fleet-summary.json": assessment,
	...
	if err := os.WriteFile(filepath.Join(dir, "imported-fleet-summary.md"), []byte(SummaryMarkdown(assessment)), 0o644); err != nil {
		return err
	}
...
func SummaryMarkdown(assessment Assessment) string {
	...
	fmt.Fprintf(&b, "ClearCutt did not build this image fleet. Missing evidence is a governance gap, not proof that an image is insecure.\n\n")
