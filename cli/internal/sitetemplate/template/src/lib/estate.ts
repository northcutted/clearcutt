...
const DATA_ROOTS = [
  // A fork's own estate, generated by `clearcutt graph build` / `graph layers`
  // and staged into the site. These win, so a real estate always displaces the
  // demo.
  path.resolve(process.cwd(), 'public/estate'),
  path.resolve(process.cwd(), 'src/data/estate'),
  // The committed public-estate snapshot: 19 real public images observed from
  // live registries. It is the fallback so this repo's own site has something
  // truthful to render, rather than shipping a permanently empty estate page or
  // a hand-written mock. Referenced in place instead of copied, so the site and
  // the Go fixture tests assert against the same bytes.
  path.resolve(process.cwd(), '..', 'examples', 'public-estate'),
];
