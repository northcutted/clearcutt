...
const clearcuttSchemaName = "clearcutt-config.schema.json"
...
func compileConfigSchema(t *testing.T) *jsonschema.Schema {
	t.Helper()
	raw, err := os.ReadFile(repoRootPath("schemas", clearcuttSchemaName))
	if err != nil {
	...
	if err := compiler.AddResource(clearcuttSchemaName, doc); err != nil {
		t.Fatalf("register fleet schema: %v", err)
	}
	schema, err := compiler.Compile(clearcuttSchemaName)
	if err != nil {
...
func validateYAMLAgainstConfigSchema(t *testing.T, schema *jsonschema.Schema, rawYAML []byte) error {
	t.Helper()
...
// clearcutt.fleet.yaml against schemas/clearcutt-config.schema.json so the
...
func TestFleetConfigFileSatisfiesPublishedSchema(t *testing.T) {
	...
	wantHeader := "# yaml-language-server: $schema=./schemas/" + clearcuttSchemaName
	...
	if err := validateYAMLAgainstConfigSchema(t, schema, rawYAML); err != nil {
		t.Fatalf("%s does not satisfy schemas/%s:\n%v", DefaultConfigPath, clearcuttSchemaName, err)
	}

	// The committed file must also load through the CLI's own validation.
	if _, err := Load(repoRootPath(DefaultConfigPath)); err != nil {
		t.Fatalf("config.Load rejects the committed config: %v", err)
	}
}
