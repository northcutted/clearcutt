...
~~~bash
APP_IMAGE=ghcr.io/acme/clearcutt-template-node:1.0.0
docker build -t "$APP_IMAGE" .
docker push "$APP_IMAGE"
APP_DIGEST=$(docker buildx imagetools inspect "$APP_IMAGE" --format '{{json .Manifest.Digest}}' | tr -d '"')
docker save "$APP_IMAGE" -o clearcutt-template-node.tar
clearcutt certify clearcutt-template-node.tar --base node22-distroless --policy certification-policy.yaml --image-ref "${APP_IMAGE%:*}@${APP_DIGEST}"
~~~
