...
case "$STACK" in
  ...
  node)
    ...
    ;;
  python)
    BASE_ID_V1="python3.15-slim"
    BASE_ID_V2="python3.15-distroless"
    ;;
  go)
...
case "$STACK" in
  ...
  node)
    ...
    ;;
  python)
    if [[ "$RUN_OUT_V1" == *"Hello from Python E2E! Version: 3.15"* ]]; then
      log_success "Source application successfully verified running under Python 3.15."
    else
...
if [ "$STACK" = "core" ]; then
  ...
  case "$STACK" in
    ...
    node)
      ...
      ;;
    python)
      if [[ "$RUN_OUT_V2" == *"Hello from Python E2E! Version: 3.15"* ]]; then
        log_success "Rebased application successfully verified running under Python 3.15!"
      else
...
case "$STACK" in
  ...
  node)
    ...
    ;;
  python)
    COVERAGE_NOTES="Successfully verified Python application packaging on Python 3.15 Slim base image. Validated execution parameters, performed live base layer swap to Python 3.15 Distroless, and successfully verified script execution under Python 3.15 interpreter inside Docker. Cosign signature gates were mechanically validated via standard wrappers."
    ;;
  go)
