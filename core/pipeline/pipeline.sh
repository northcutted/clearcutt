...
certify_target() {
  ...
  #
  # Crypto VEX bridge (provenance reshape, Phase 3): when a patched-not-bumped
  # openssl/sqlite would false-positive on a version-keyed CVE, the fleet records
  # an OpenVEX not_affected statement (generated by `clearcutt remediation
  # vex-crypto` from the known-good identity allowlist). If that VEX exists, pass
  # it to Grype natively so the scanner clears the gap honestly. The .grype.yaml
  # ignore derived from the SAME VEX is the bridge for any scanner that doesn't
  # consume OpenVEX. CLEARCUTT_CRYPTO_VEX overrides the default path.
  local grype_vex_args=()
  local grype_pipeline_dir
  grype_pipeline_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  local crypto_vex="${CLEARCUTT_CRYPTO_VEX:-$grype_pipeline_dir/../vex/crypto.openvex.json}"
  if [[ -f "$crypto_vex" ]]; then
    grype_vex_args=(--vex "$crypto_vex")
    log_info "Crypto VEX present; passing $crypto_vex to Grype (--vex)."
  fi
  ...
  local grype_exit=0
  if grype "sbom:$sbom_path" --fail-on high --only-fixed "${grype_vex_args[@]}" -o json > "$scan_path"; then
