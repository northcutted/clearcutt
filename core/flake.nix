...

{
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      # clearcutt.yaml by `clearcutt fleet compile` into
      ...
      let
        ...
        # `clearcutt service scaffold` from clearcutt.yaml.
        serviceSpecs = import ./lib/service-extensions.nix {};

        # Fork-configurable output identity generated from clearcutt.yaml.
        ...
        assertRecipe = cell:
          ...
          then cell
          else throw "clearcutt fleet-matrix: cell ${cell.line}-${cell.tier} declares language=${cell.language} version=${cell.version} with no recipe in lib/registry.nix; add it there or remove the line from clearcutt.yaml matrix.languages";
        ...
        # clearcutt.yaml. An unselected recipe that stopped evaluating —
    ...
    in
    {
      ...
      checks = forLinuxSystems (system:
        let
          ...
          # lists from lib/fleet-matrix.nix (compiled from clearcutt.yaml).
